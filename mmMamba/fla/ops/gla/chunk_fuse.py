...
"""def fused_chunk_gla_new(
    ...
    new_g=ComputeAttnParallelFunction_Nochunk.apply(q, g)

    A=torch.matmul((q*torch.exp(new_g)*scale), (k*torch.exp(-new_g)).transpose(2,3))
    causal_mask_np = np.tril(np.ones((seq_len, seq_len)))
    causal_mask = torch.from_numpy(causal_mask_np).to(A.device)
    masked_attention = torch.where(causal_mask == 1, A, torch.tensor(0.0, device=A.device))
    o = torch.matmul(masked_attention, v)
    o = o[..., :seq_len, :]

    return o, None

class ComputeAttnParallelFunction_Nochunk(torch.autograd.Function):
    ...
    def forward(ctx, q, g):
        ...
        # cumulative decay should be in float32, otherwise the err will be accumulated and amplified.
        g = torch.empty_like(g, dtype=torch.float32)
        # cumulative decay should be in float64, otherwise the err will be accumulated and amplified.
        batch_size, n_heads, seq_len, d_head_qk = q.shape
        scale = q.shape[-1] ** -0.5
        ctx.scale = scale
